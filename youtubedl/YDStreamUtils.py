...
def downloadProgressCallback(prog, data):
    if not hasattr(data, 'info'):
        return prog.update(0, '', data)
    line1 = os.path.basename(data.info.get('filename', ''))
    line2 = []
    if data.speedStr:
        line2.append(data.speedStr)
    if data.etaStr:
        line2.append('{0}: {1}'.format('', data.etaStr))
    line2 = ' - '.join(line2)
    line3 = []
    total = data.info.get('total_bytes')
    if total:
        line3.append('{0}: {1}'.format('', simpleSize(total)))
    downloaded = data.info.get('downloaded_bytes')
    if downloaded:
        line3.append('{0}: {1}'.format('', simpleSize(downloaded)))
    line3 = ' - '.join(line3)
...
def getDownloadPath(use_default=None):
    ...
    if path:
        if not use_default:
            new = xbmcgui.Dialog().yesno('', '' + ' ' + path + '. ' + '', nolabel='', yeslabel='')
            if new:
                path = ''
    if not path:
        path = xbmcgui.Dialog().browse(3, '', 'files', '', False, True)
    if not path:
        return
